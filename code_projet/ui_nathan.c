...
void draw_header_window(WINDOW* header_window, char *tab_files[], int current_tab, int total_tabs) {
...
int start_user_interface(char* tab_files[], int tab_count) {
    /* Initialize ncurses */
    initscr();
...
int main() {
    /* Example data - files to display in tabs */
    char* file_list[] = {"exemple_txt.txt", "test.txt"};
    int file_count = 2;

    /* Start the user interface */
    return start_user_interface(file_list, file_count);
}
